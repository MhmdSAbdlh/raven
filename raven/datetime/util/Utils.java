...
public class Utils {
	...
	public static Point adjustPopupLocation(JPopupMenu popupMenu, Component component, Point space) {
		...
		int x = frameInsets.left + rightSpace > 0 ? locationOnFrame.x + space.x
				: Math.max(locationOnFrame.x + component.getWidth() - popupSize.width - space.x, frameInsets.left);
		int y = frameInsets.top + bottomSpace > 0 ? locationOnFrame.y + space.y
				: Math.max(locationOnFrame.y - component.getHeight() - popupSize.height - space.y, frameInsets.top);
	...
	public static Icon createIcon(String icon, float scale) {
		return new FlatSVGIcon(icon, scale)
				.setColorFilter(new FlatSVGIcon.ColorFilter((component, color) -> UIManager.getColor("Actions.Grey")));
	}
}