...
public class DatePicker extends PanelPopupEditor
	...
	private static String[] defaultPanel = null;
	...
	public void setDateFormat(String format) {
		...
		if (!this.dateFormatPattern.equals(format)) {
			...
			if (editor != null) {
				InputUtils.changeDateFormatted(editor, format,
						getDateSelectionMode() == DateSelectionMode.BETWEEN_DATE_SELECTED, separator,
						getInputValidationListener());
				this.defaultPlaceholder = null;
			}
	...
	public void setUseCustomPanelOption(boolean useMyPanelOption, String today, String yesterday, String last_week,
			String last_month, String last_season, String last_year, String custom) {
		if (this.useMyPanelOption != useMyPanelOption) {
			this.useMyPanelOption = useMyPanelOption;
			if (useMyPanelOption) {
				if (panelDateMoh == null) {
					panelDateMoh = new PanelDateMoh(this);
					panelDateMoh.installDateOptionLabel(today, yesterday, last_week, last_month, last_season, last_year
							);
				}
				add(panelDateMoh, "dock east,gap 0 10 10 10");
				repaint();
				revalidate();
			} else {
				if (panelDateMoh != null) {
	...
	public void setUseMyCustomPanelOption(boolean useMyPanelOption) {
		if (this.useMyPanelOption != useMyPanelOption) {
			this.useMyPanelOption = useMyPanelOption;
			if (useMyPanelOption) {
				if (panelDateMoh == null) {
					panelDateMoh = new PanelDateMoh(this);
					panelDateMoh.installDateOptionLabel();
				}
				add(panelDateMoh, "dock east,gap 0 10 10 10");
				repaint();
				revalidate();
	...
	private void installEditor(JFormattedTextField editor) {
		if (editor != null) {
			...
			editorButton = new JButton(
					editorIcon != null ? editorIcon : Utils.createIcon("raven/datetime/icon/calendar.svg", 0.38f));
			...
			InputUtils.useDateInput(editor, dateFormatPattern,
					getDateSelectionMode() == DateSelectionMode.BETWEEN_DATE_SELECTED, separator, getValueCallback(),
					getInputValidationListener());
			setEditorValue();
	...
	public static void setDefaultPanel(String[] defaultPanel) {
		if (defaultPanel == null) {
			DatePicker.defaultPanel = null;
		} else {
			DatePicker.defaultPanel = Arrays.copyOf(defaultPanel, defaultPanel.length);
		}
	}
	...
	public static String[] getDefaultPanel() {
		...
		return Arrays.copyOf(defaultPanel, defaultPanel.length);
	}
