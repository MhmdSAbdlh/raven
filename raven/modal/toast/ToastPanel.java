...
public class ToastPanel extends JPanel {
	...
	private void init() {
		...
		putClientProperty(FlatClientProperties.STYLE,
				"" + "[light]background:" + styleLight + ";" + "[dark]background:" + styleDark + ";");
	}
	...
	protected void initBorder() {
		...
		int borderWidth = borderStyle.getBorderType() == ToastBorderStyle.BorderType.OUTLINE
				? borderStyle.getBorderWidth()
				: 0;
		if (getOption().isHeavyWeight()) {
			...
			if (FlatUIUtils.isInsetsEmpty(borderStyle.getShadowSize()) && borderStyle.getRound() == 0
					&& borderWidth == 0) {
				setBorder(border);
				borderSpace = null;
			} else {
				DropShadowBorder shadow = new DropShadowBorder(borderStyle.getShadowSize(),
						borderStyle.getShadowOpacity(), borderStyle.getShadowColor(), borderWidth,
						toastData.getBorderColor(), borderStyle.getRound());
	...
	private String getLayoutInsets() {
		...
		return String.format(Locale.ROOT, "insets %d %d %d %d", padding.top + add, padding.left + add,
				padding.bottom + add, padding.right + add);
	}
	...
	private String getLayoutColumn(ThemesData themesData) {
		...
		if (toastPromise == null && (toastData.getOption().getStyle().getCustomIcon() == null && themesData.icon == null
				|| !toastData.getOption().getStyle().isShowIcon())) {
	...
	private void installDefault(ThemesData themesData) {
		content = new ToastContent(new MigLayout("filly," + getLayoutInsets(), getLayoutColumn(themesData), "[center]"),
				toastData);
	...
	private void installStyle() {
		...
		if (toastData.getOption().getStyle().getBackgroundType() == ToastStyle.BackgroundType.DEFAULT) {
			putClientProperty(FlatClientProperties.STYLE,
					"" + "[light]background:mix(" + styleLight + ",$TextArea.background,10%);" + "[dark]background:mix("
							+ styleDark + ",$TextArea.background,10%);");
		}
		if (textMessage != null && toastData.getOption().getStyle().isPaintTextColor()) {
			textMessage.putClientProperty(FlatClientProperties.STYLE,
					"" + "[light]foreground:" + titleLight + ";" + "[dark]foreground:" + titleDark + ";");
		}
		if (labelTitle != null) {
			labelTitle.putClientProperty(FlatClientProperties.STYLE, "" + "font:bold;" + "[light]foreground:"
					+ titleLight + ";" + "[dark]foreground:" + titleDark + ";");
		}
	}
	...
	private JButton createCloseButton() {
		...
		buttonClose.putClientProperty(FlatClientProperties.STYLE, "" + "arc:999;" + "margin:3,3,3,3;" + "borderWidth:0;"
				+ "focusWidth:0;" + "innerFocusWidth:0;" + "background:null;");
	...
	public boolean checkSameLayout(ToastLayoutOption layoutOption) {
		...
		if (option.getLocationSize() != null && layoutOption.getLocationSize() != null) {
			return option.getLocationSize().getX().floatValue() == layoutOption.getLocationSize().getX().floatValue()
					&& option.getLocationSize().getY().floatValue() == layoutOption.getLocationSize().getY()
							.floatValue();
		}
		return false;
	}
