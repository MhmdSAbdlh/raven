...
public class ToastLayout implements LayoutManager {
		...
		synchronized (parent.getTreeLock()) {
			...
			for (int i = 0; i < count; i++) {
				...
				if (component instanceof ToastPanel) {
					...
					Rectangle rec = OptionLayoutUtils.getLayoutLocation(parent, toastPanel, toastPanel.getAnimate(),
							option);
					int index = i;
					int y = rec.y;
					if (index > 0) {
						y = getY(components, toastPanel, index, option.getVerticalLocation(), rec, baseMargin,
								isVerticalSlide, toastPanel.getAnimate());
					} else {
	...
	private int getY(Component[] components, ToastPanel parentPanel, int index, Location vertical, Rectangle rec,
			Insets baseMargin, boolean isVerticalSlide, float animate) {
