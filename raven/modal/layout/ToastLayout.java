...
import raven.modal.toast.ToastPanel.ToastData;
...
public class ToastLayout implements LayoutManager {
		...
		synchronized (parent.getTreeLock()) {
			...
			for (int i = 0; i < count; i++) {
				...
				if (component instanceof ToastPanel) {
					...
					ToastData data = toastPanel.getToastData();
					ToastOption option;
					if (data == null)
						option = ToastOption.getDefault();
					else
						option = data.getOption();
					LayoutOption layoutOption = option.getLayoutOption().createLayoutOption(parent,
							toastPanel.getOwner());
					float animate = toastPanel.getEasingAnimate();
					Rectangle rec = OptionLayoutUtils.getLayoutLocation(parent, null, toastPanel, animate,
							layoutOption);
					int y = rec.y;
					if (i > 0) {
	...
	private float getLayoutY(Container parent, Dimension comSize, LayoutOption layoutOption) {
		int insets = parent.getInsets().top + parent.getInsets().bottom
				+ UIScale.scale(layoutOption.getMargin().top + layoutOption.getMargin().bottom);
		int height = parent.getHeight() - insets;
		Point point = OptionLayoutUtils.location(0, height, comSize, layoutOption.getLocation(),
				layoutOption.isOverflowAlignmentAuto());
		return point.y;
	}

	private int getY(Component[] components, ToastPanel parentPanel, ToastLayoutOption layoutOption, int index,
			Rectangle rec, Insets baseMargin, float ly, float animate) {
