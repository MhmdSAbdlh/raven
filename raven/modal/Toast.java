...
public class Toast {
	...
	private Toast() {
		...
		defaultOption.getLayoutOption().setLocation(Location.CENTER, Location.TOP).setAnimateDistance(0, -1f);
	}

	private void initThemesData() {
		themesDataMap = new HashMap<>();
		themesDataMap.put(Type.SUCCESS,
				new ToastPanel.ThemesData("raven/modal/icon/toast_success.svg", new String[] { "#1EA97C", "#1EA97C" }));
		themesDataMap.put(Type.INFO,
				new ToastPanel.ThemesData("raven/modal/icon/toast_info.svg", new String[] { "#3B82F6", "#3B82F6" }));
		themesDataMap.put(Type.WARNING,
				new ToastPanel.ThemesData("raven/modal/icon/toast_warning.svg", new String[] { "#CC8925", "#CC8925" }));
		themesDataMap.put(Type.ERROR,
				new ToastPanel.ThemesData("raven/modal/icon/toast_error.svg", new String[] { "#FF5757", "#FF5757" }));
		themesDataMap.put(Type.BIRTHDAY, new ToastPanel.ThemesData("raven/modal/icon/toast_birthday.svg",
				new String[] { "#CCC925", "#CCC925" }));
	...
	public static void show(Component owner, Type type, String message, ToastLocation location, ToastOption option) {
		...
		option.getLayoutOption().setLocation(l.getHorizontalLocation(), l.getVerticalLocation())
	...
	public static void showPromise(Component owner, String message, ToastLocation location, ToastOption option,
	...
	private static void show(Component owner, Type type, Object object, ToastOption option, ToastPromise promise) {
		...
		ToastPanel toastPanel = new ToastPanel(toastContainerLayer,
				new ToastPanel.ToastData(type, option, themesData, message));
		...
		if (rootPaneContainer.getRootPane().getComponentOrientation().isLeftToRight() != toastPanel
	...
	private ToastContainerLayer getToastContainerLayered(RootPaneContainer rootPaneContainer) {
		...
		if (map.containsKey(rootPaneContainer)) {
			...
			if (rootPaneContainer.getRootPane().getComponentOrientation().isLeftToRight() != toastContainerLayer
					.getComponentOrientation().isLeftToRight()) {
				toastContainerLayer
						.applyComponentOrientation(rootPaneContainer.getRootPane().getComponentOrientation());
			}
	...
	private void uninstall(RootPaneContainer rootPaneContainer) {
		if (map.containsKey(rootPaneContainer)) {
			...
			JLayeredPane windowLayeredPane = rootPaneContainer.getLayeredPane();
			rootPaneContainer.getRootPane().removePropertyChangeListener("ancestor",
					(PropertyChangeListener) toastContainerLayer.getPropertyData());
	...
	public enum Type {
		DEFAULT, SUCCESS, INFO, WARNING, ERROR, BIRTHDAY
	}
}
